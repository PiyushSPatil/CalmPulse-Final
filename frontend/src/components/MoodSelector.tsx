...
export function MoodSelector({ selectedMood = null, onSelect, readOnly = false }: MoodSelectorProps) {
  const [selected, setSelected] = useState<number | null>(selectedMood);

  useEffect(() => {
    setSelected(selectedMood ?? null);
  }, [selectedMood]);
